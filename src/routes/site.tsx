...
import { createStrongEtag, hasMatchingIfNoneMatch } from "../helpers/http-cache.ts";
import { formatNsiteSubdomain } from "../helpers/nsite-host.ts";
import { getManifestServers, resolveManifestPath } from "../helpers/site-manifest.ts";
...
function appendOnionLocation(headers: Headers, pubkey?: string, identifier = "") {
  if (!ONION_HOST) return;
  const url = new URL(ONION_HOST);
  const subdomain = pubkey ? formatNsiteSubdomain(pubkey, identifier) : undefined;
  if (subdomain) {
...
export async function handleSiteRequest(c: Context, site: AddressPointer): Promise<Response> {
  ...
  const [userServers, manifest] = await Promise.all([getUserBlossomServers(pubkey, 10_000), getManifest(site, 10_000)]);

  if (!manifest) {
    return c.html(html` <!DOCTYPE html>${(<SiteNotFound hostname={url.hostname} />)} `, 404);
  }

  const match = resolveManifestPath(manifest, url.pathname);
  if (!match) {
    return c.html(html` <!DOCTYPE html>${(<PathNotFound hostname={url.hostname} pathname={url.pathname} />)} `, 404);
  }
  ...
  const servers = mergeBlossomServers(manifestServers, userServers, BLOSSOM_SERVERS);
  ...
  if (!upstream) {
    return new Response("Bad Gateway: Unable to retrieve the requested file from storage servers.", {
      status: 502,
    });
  }
  ...
  headers.set("content-type", mime || upstream.headers.get("content-type") || "application/octet-stream");
  ...
  headers.set("ETag", etag);
  headers.set("Cache-Control", "public, max-age=3600");
  headers.set("Last-Modified", upstream.headers.get("last-modified") || getSiteLastModified(manifest.created_at));
  appendOnionLocation(headers, pubkey, identifier);

  // Set response status
  const status = match.is404 ? 404 : upstream.status === 206 ? 206 : upstream.ok ? 200 : upstream.status;
