...
async function getHomeSites(host: string, protocol: string): Promise<HomeSite[]> {
  ...
  const profileEntries = await Promise.all(
    [...uniquePubkeys].map(async (pubkey) => [pubkey, await getUserProfile(pubkey)] as const),
  );
  ...
  return (
    <li class="site-card">
      <div>
        ...
        {site.identifier ? <span class="site-meta"> &middot; {site.identifier}</span> : null}
      </div>
      {site.description ? <div class="site-description">{site.description}</div> : null}
      <div class="site-meta">
        by {site.authorName || site.npub} &middot;{" "}
        {pluralize(site.pathCount, "page", "pages")} &middot; updated {formatAgeFromUnix(site.createdAt)} ago
        {" "}&middot; <a href={statusHref}>status</a>
      </div>
    </li>
  );
};
    ...
    <html lang="en">
      ...
      <body>
        <main>
          ...
          {sites.length === 0 ? (
            <p class="empty-state">No sites cached yet. Sites will appear here as they are visited.</p>
          ) : (
            <ul class="site-list">
              {sites.map((site) => (
...
export async function homeRoute(c: Context): Promise<Response> {
  ...
  return c.html(html`<!DOCTYPE html>${(<HomePage sites={sites} host={url.host} />)}`, 200, {
